...
public class sudoku {
	...
	public static void puller() throws InterruptedException, IOException, AWTException {
		...
		driver.get(
				"https://www.nytimes.com/puzzles/sudoku/medium");// getting link
		...
		for (int u = 0; u < 4; u++) {
			...
			for (int i = 0; i < 81; i++) {
				...
				if (blocks.get(i).blockPossibles.size() == 1 && !blocks.get(i).set) {
					...
					driver.findElement(
							By.xpath("//*[@id=\"pz-game-root\"]/div[2]/div/div[1]/div/div/div/div[" + (i + 1) + "]"))
							.click();
				...
				for (int h = 1; h < 10; h++) {// each number possible
					...
					if (counterList.size() == 1) {
						if (blocks.get((9 * i) + counterList.get(0)).num == 0) {
							System.out.println("Horizontal setting: " + h + " at: " + ((9 * i) + counterList.get(0))
									+ " in: " + blocks.get((9 * i) + counterList.get(0)).blockPossibles);
							driver.findElement(By.xpath("//*[@id=\"pz-game-root\"]/div[2]/div/div[1]/div/div/div/div["
									+ ((i * 9) + counterList.get(0) + 1) + "]")).click();
				...
				for (int h = 1; h < 10; h++) {// each number possible
					...
					if (counterList.size() == 1) {
						if (blocks.get(i + (9 * counterList.get(0))).num == 0) {
							System.out.println("Vertical setting: " + h + " at: " + ((counterList.get(0) * 9) + i)
									+ " in: " + blocks.get((counterList.get(0) * 9) + i).blockPossibles);
							driver.findElement(By.xpath("//*[@id=\"pz-game-root\"]/div[2]/div/div[1]/div/div/div/div["
									+ (((counterList.get(0) * 9) + i) + 1) + "]")).click();
				...
				for (int h = 1; h < 10; h++) {// each number possible
					...
					if (counterList.size() == 1) {
						if (blocks.get(aList.get(i).get(counterList.get(0))).num == 0) {
							System.out.println("Square setting: " + h + " at: " + (aList.get(i).get(counterList.get(0)))
									+ " in: " + blocks.get(aList.get(i).get(counterList.get(0))).blockPossibles);
							driver.findElement(By.xpath("//*[@id=\"pz-game-root\"]/div[2]/div/div[1]/div/div/div/div["
									+ (aList.get(i).get(counterList.get(0)) + 1) + "]")).click();
			...
			// If there is x amount of possible numbers that are the same for x amount of squares then you can remove blockpossibles of x's x amount of 
